...
import { dummyData, iconMapping } from './data';

const capitalizeFirstLetter = (string) => {
  return string.charAt(0).toUpperCase() + string.slice(1).toLowerCase();
};
export const TabsSpotlightSearch = (props) => {
  ...
  useEffect(() => {
    ...
    const tabIndex = categoriesWithResults.indexOf(firstCategoryWithResults);
    if (tabIndex !== -1) {
  ...
  const getTabContent = (items, category) => {
    switch (category) {
      case 'images':
        return (
          <Box
                    ...
                    (category, index) =>
                      index === selectedTab && (
                        <Box
                          ...
                          key={category}
                        >
                          {getTabContent(groupedItems[category], category)}
                        </Box>
                      )
                  )}
                </>
